...
/**
 *   	\file       estadisticas_list.php
 *		\ingroup    test
 *		\brief      List page for estadisticas
 */
...
// load test libraries
require_once __DIR__.'/class/estadisticas.class.php';
...
$hookmanager->initHooks(array('estadisticaslist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('test', 'estadisticas', 'write');
	$permissiontodelete = $user->hasRight('test', 'estadisticas', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("test")) {
	accessforbidden('Module test not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Estadisticas';
	$objectlabel = 'Estadisticas';
	$uploaddir = $conf->test->dir_output;
...
//$help_url = "EN:Module_Estadisticas|FR:Module_Estadisticas_FR|ES:Módulo_Estadisticas";
$help_url = '';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/test/estadisticas_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/test/estadisticas_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);
*/
// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEstadisticasRef";
$modelmail = "estadisticas";
$objecttmp = new Estadisticas($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_test', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
